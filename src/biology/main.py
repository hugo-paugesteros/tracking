...
da_raw = None
layer_green, layer_red, layer_blue, layer_roi = None, None, None, None
...
layer_green = viewer.add_points(name="Green Points", border_color="orange", **kwargs)
...
def predict_all_frames_widget(
    ...
    for ch in channels_to_process:
        ...
        if not f.empty:
            ...
            print(f" -> Success: Rendered {len(new_coords)} points to the {ch} layer.")
        else:
...
def link_and_track_widget(
    ...
    if N == 1:
        ...
        pheno_float_list = [pheno_to_float[p] for p in t_filtered["Track_Phenotype"]]

        rgba_colors = [transform_color(master_color_dict[k])[0] for k in active_phenos]
        controls = np.linspace(0, 1, N)
        custom_cmap = Colormap(colors=rgba_colors, controls=controls, name="exact_cmap")
    ...
    for _, row in t_filtered.iterrows():
        z, y, x, slice_pheno = row["frame"], row["y"], row["x"], row["Slice_Phenotype"]
        coord = [z, y, x]
